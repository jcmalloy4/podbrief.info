...
// Read all unique podcasts from our briefs
const briefsDir = path.join(__dirname, '..', 'podbrief.info', 'briefs');
const allPodcasts = new Set();

// Scan all briefs to find unique podcast names
console.log('Scanning briefs for podcasts...');
...
for (const podcastId of podcastDirs) {
  ...
  for (const episode of episodes.slice(0, 1)) { // Just check first episode in each dir
    const filePath = path.join(episodesPath, episode);
    const content = fs.readFileSync(filePath, 'utf8');
    const match = content.match(/<p class="podcast-name">([^<]+)<\/p>/);
    if (match) {
      allPodcasts.add(match[1]);
    }
  }
}

console.log(`Found ${allPodcasts.size} unique podcasts\n`);
...
const podcastsOutDir = path.join(__dirname, '..', 'podbrief.info', 'podcasts');
if (!fs.existsSync(podcastsOutDir)) {
  fs.mkdirSync(podcastsOutDir, { recursive: true });
}

let totalPages = 0;
let totalBriefs = 0;
const sortedPodcasts = Array.from(allPodcasts).sort();
...
for (const podcast of sortedPodcasts) {
  ...
  const outputPath = path.join(podcastsOutDir, `${slug}.html`);
